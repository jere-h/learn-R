...
cel$Gender <- recode(cel$female, `0`="Male", `1`="Female")

cel %>% filter(congress==115) %>% ggplot(aes(x=dwnom1,y=all_pass,color=Gender)) + geom_point() + labs(x="Ideology",y="Bills Passed")
...
cel$Gender <- recode(cel$female, `0`="Male", `1`="Female")
cel$rec_majority <- recode(cel$majority, `0`="Minority",`1`="Majority")

cel %>% filter(congress==115) %>% ggplot(aes(x=votepct,y=all_pass,color=Gender))+geom_point()+labs(x="Vote Percentage",y="Bills Passed")+scale_color_manual(values=c("orange","green"))+facet_wrap(~rec_majority)
```
...
cel %>% filter(congress==115) %>% ggplot(aes(x=rec_majority,y=les)) + geom_boxplot() + labs(x="Majority or Minority", y="Legislative Effectiveness",title="LES in the 115th Congress")

...
```{r,problem4}
dat$faminc_new %>% boxplot(main="Jeremy", xlab="Family Income")
```
...
dat %>% filter(gender==1 & region<=2 & marstat==1 & newsint==1) %>% str()
```
...
filtered_dat <- dat %>% filter(gender==1 & region<=2 & marstat==1 & newsint==1)
filtered_dat$investor %>% table()
``` 
...
rec_tax1 <- recode(dat$CC18_325a,`1`=1, `2`=0)
rec_tax2 <- recode(dat$CC18_325d,`1`=1, `2`=0)
dat$rec_tax1 <- rec_tax1
dat$rec_tax2 <- rec_tax2

dat <- mutate(dat, tax_scale=rec_tax1+rec_tax2)
dat$tax_scale[1:20]
```
...
```{r problem10}
dat %>% filter(investor==2 & faminc_new>=5 & faminc_new<=10) %>% summarise("Mean Religion Imp."=mean(pew_religimp),"Median Religion Imp."=median(pew_religimp),"Standard Dev. Religion Imp."=sd(pew_religimp))
```
...
```{r problem11}
dat %>% summarise(mean(faminc_new), median(faminc_new), sd(faminc_new)) %>% kable(col.names=c("Mean","Median","Std. Dev."))
```
